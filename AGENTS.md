...
- Put each skill in `skills/<category>/<skill-name>/SKILL.md` (or directly in `skills/<skill-name>/SKILL.md` when no category is requested), with matching `name` and a meaningful `description` in YAML frontmatter. Keep the repository root free of a `SKILL.md` so it remains a collection.
...
- Update the README’s skill catalog when adding or moving a skill. Keep installation instructions accurate. No package manifest or build pipeline is required for ordinary instruction-only skills.
